...
def min_func(params):
    ...
    # Get the heston model EUCall prices for 128 strike prices - corresponds to 128 V(0,S(0))
    options_model = Heston_EuCall(S0, r, gam0, kappa, lamb, sig_tild, T=1, K=strikes, R=1.5, N=2 ** 15)
    # minimize mean of the square of the function - objective function must return a scalar hence the mean
    # find the parameters such that they align with the minimimization of mean squared error between the true option value and the option value generated by EUCall Heston model
